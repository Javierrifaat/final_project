...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_event'])) {
        ...
        $event_format = $_POST['event_format'] === 'team' ? 1 : 0;  
        $team_size = isset($_POST['team_size']) ? $_POST['team_size'] : null; // Jika tim, jumlah anggota tim
        ...
        $query = "INSERT INTO event (event_name, event_description, registration_fee, rules, location, event_date, event_time, image_url, event_format, team_size) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($db, $query);
        mysqli_stmt_bind_param($stmt, 'ssdsssssii', $name, $description, $price, $rules, $location, $event_date, $event_time, $image_url, $event_format, $team_size);
        mysqli_stmt_execute($stmt);

        // Ambil ID event yang baru saja dimasukkan
        $event_id = mysqli_insert_id($db);

        // Jika event menggunakan tim, buat tim secara dinamis
        if ($event_format == 1) { // 1 berarti tim
            // Ambil nama event untuk membuat nama tim yang unik
            $event_name = $_POST['name'];  // Nama event yang diambil dari form
            $team_count_query = "SELECT COUNT(*) FROM teams WHERE event_id = ?";
            $stmt = mysqli_prepare($db, $team_count_query);
            mysqli_stmt_bind_param($stmt, 'i', $event_id);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_bind_result($stmt, $team_count);
            mysqli_stmt_fetch($stmt);
            mysqli_stmt_close($stmt);
        
            // Buat nama tim berdasarkan nama event dan nomor urut
            $team_name = $event_name . " - Team " . ($team_count + 1);
        
            // Insert team dengan nama unik
            $insert_team_query = "INSERT INTO teams (team_name, event_id) VALUES (?, ?)";
            $stmt = mysqli_prepare($db, $insert_team_query);
            mysqli_stmt_bind_param($stmt, 'si', $team_name, $event_id);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
        }
        ...
        // Hapus tim yang terhubung dengan event
        $query = "DELETE FROM teams WHERE event_id = ?";
        $stmt = mysqli_prepare($db, $query);
        mysqli_stmt_bind_param($stmt, 'i', $eventId);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
...
createParticles();
    ...
    <div class="add-event-container">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                <label for="event_format" class="form-label"><i class="fas fa-users"></i> Format Lomba</label>
                <select class="form-select" id="event_format" name="event_format" required>
            ...
            <div class="mb-3" id="team_size_container" style="display: none;">
                <label for="team_size" class="form-label">Ukuran Tim</label>
                <input type="number" class="form-control" id="team_size" name="team_size" placeholder="Jumlah anggota tim" min="2" max="10">
            </div>
